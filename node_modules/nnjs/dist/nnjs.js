...
if (!Float64Array.prototype.fill) {
  Float64Array.prototype.fill = function (value) {
...
var Blob = function Blob(width, height, depth) {
  var num = arguments.length <= 3 || arguments[3] === undefined ? 1 : arguments[3];
...
var ConvolutionLayer = function ConvolutionLayer(outputDepth, windowSize, stride, pad, biasFiller) {
...
ConvolutionLayer.prototype.init = function (inputWidth, inputHeight, inputDepth) {
  ...
  for (var i = 0; i < this.outputDepth; i++) {
    if (this.biasFiller) {
      ...
      this.biases.data[i] = _rand2['default'].randn(0, 1);
    }
  }

  for (var i = 0; i < this.windowSize * this.windowSize * inputDepth * this.outputDepth; i++) {
    this.weights.data[i] = _rand2['default'].randn(0, 1.0 / Math.sqrt(this.windowSize * this.windowSize * inputDepth));
  }
          ...
          for (var y = startY; y < startY + this.windowSize; y++) {
            if (x >= 0 && x < inputBlob.width && y >= 0 && y < inputBlob.height) {
              for (var id = 0; id < inputBlob.depth; id++) {
                val += this.weights.data[d * this.weights.depth * this.weights.width * this.weights.height + id * this.weights.width * this.weights.height + (x - startX) * this.weights.height + (y - startY)] * inputBlob.data[id * inputBlob.width * inputBlob.height + x * inputBlob.height + y];
              }
            }
          }
        }
          ...
          for (var y = startY; y < startY + this.windowSize; y++) {
            if (x >= 0 && x < this.inputBlob.width && y >= 0 && y < this.inputBlob.height) {
              for (var id = 0; id < this.inputBlob.depth; id++) {
                var weightIdx = d * this.weights.depth * this.weights.width * this.weights.height + id * this.weights.width * this.weights.height + (x - startX) * this.weights.height + (y - startY);
...
var PoolingLayer = function PoolingLayer(poolingFunc, windowSize, stride, pad) {
...
var ReLULayer = function ReLULayer() {};
...
var SigmoidLayer = function SigmoidLayer() {};
...
var TanhLayer = function TanhLayer() {};
...
var LinearLayer = function LinearLayer(numOutput, biasFiller) {
  this.outputWidth = 1;
  this.outputHeight = 1;
...
LinearLayer.prototype.init = function (inputWidth, inputHeight, inputDepth) {
  ...
  for (var i = 0; i < this.outputDepth * this.numInput; i++) {
    this.weights.data[i] = _rand2['default'].randn(0, 1.0 / Math.sqrt(this.numInput));
  }
...
var RegressionLayer = function RegressionLayer() {
  this.inputBlob = null;
...
var SoftmaxLayer = function SoftmaxLayer() {
  this.inputBlob = null;
...
function Trainer(network, iterations, batchSize, learningRate) {
  var weightDecay = arguments.length <= 4 || arguments[4] === undefined ? 0.0 : arguments[4];
  var momentum = arguments.length <= 5 || arguments[5] === undefined ? null : arguments[5];
  var displayIterations = arguments.length <= 6 || arguments[6] === undefined ? 0 : arguments[6];
  var testIterations = arguments.length <= 7 || arguments[7] === undefined ? 0 : arguments[7];
...
Trainer.prototype.train = function (trainVectors, trainLabels, testVectors, testLabels) {
  ...
  for (var n = 0; n < this.iterations; n++) {
    if (this.displayIterations && n % this.displayIterations === 0) {
      console.log("n: " + n);
      console.log("Loss: " + lossArr.reduce(function (prev, cur) {
        return prev + cur;
      }, 0) / lossArr.length);
    }
...
Trainer.prototype.test = function (testVectors, testLabels) {
  ...
  for (var i = 0; i < testVectors.length; i++) {
    if (_util2['default'].argmax(this.network.fprop(testVectors[i]).data) === testLabels[i]) {
      correct++;
    }
  }
...
exports['default'] = { Blob: Blob, ConvolutionLayer: ConvolutionLayer, PoolingLayer: PoolingLayer, ReLULayer: ReLULayer, SigmoidLayer: SigmoidLayer,
  TanhLayer: TanhLayer, LinearLayer: LinearLayer, RegressionLayer: RegressionLayer, SoftmaxLayer: SoftmaxLayer, Network: Network,
  Trainer: Trainer };
